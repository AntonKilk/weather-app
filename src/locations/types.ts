// Location and slot types, plus Open-Meteo geocoding domain types.
//
// `default` slots are seeded at build time from `VITE_DEFAULT_LOCATIONS`
// (STORY-005); `custom` slots are user-added via geocoding autocomplete
// (STORY-008/009) and may be empty until the user picks something.
//
...
//
// The geocoding field set was spike-verified against the Open-Meteo endpoint
// on 2026-06-07 (see .agents/PRDs/offline-weather-pwa.prd.md). The endpoint:
 ...
 * This is the boundary between STORY-008 (geocoding autocomplete) and
 * STORY-009 (custom slot persistence) — STORY-009 reads exactly these three
 * fields. Structurally identical to `Location`; kept as a separate alias for
 * intent clarity at the autocomplete boundary.
 */
export type LocationSelection = Location;
