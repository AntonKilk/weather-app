...
## Phase 8: RUSSIAN-LANGUAGE CHAT SUMMARY (always)

**This phase is mandatory. Print it in chat AFTER the Phase 7 block, every time, regardless of status (COMPLETE / INCOMPLETE / NEEDS_INPUT).** This is for the owner — keep it concise, conversational, and entirely in Russian (Cyrillic). No mixed-language headers.
...
{2–4 пункта или короткий абзац: что реально появилось в коде / репозитории. Назови ключевые файлы и функции, не пересказывай весь отчёт.}
...
| UPDATE | `path/to/changed.ts` |
| ARCHIVE | план в `.agents/plans/completed/` |
...
- `{typecheck}` → exit 0
- `{tests}` → N passed
- `{build}` → exit 0 (если запускался)
- Verifier round {n} → **CONFIRMED** / **REFUTED** / **INCOMPLETE**

### GitHub

- Issue #N {закрыт с комментарием / комментарий добавлен / не привязан}.
- Ветка `{branch}` запушена.
- `master` обновлён фаст-форвардом на `{sha}` / **публикация заблокирована: {причина}**.

---

## Что нужно проверить тебе

{Нумерованный список реальных действий для владельца. Каждый пункт — конкретная проверка, не общая формулировка.}

1. **{Что именно}** — {почему это нужно, и команда/чек-лист, если применимо}.
2. ...
```

**Правила Phase 8**:

- Полностью на русском. Заголовки, таблицы, маркированные пункты — всё. Английский допустим только для имён файлов, команд, технических идентификаторов (`fetchForecast`, `npm test`, SHA, имя ветки).
- Раздел «**Что нужно проверить тебе**» — обязателен, даже если кажется, что проверять нечего. Минимум: пробежать глазами отчёт и AC-маппинг, проверить, что в коммитах нет того, чему там быть не должно (секреты, личные данные). Если был `DEFERRED — owner` пункт в AC-маппинге — он сюда ОБЯЗАН попасть с конкретной командой/чек-листом.
- Не дублировать дословно Phase 7 — это краткое summary для человека, не для парсера. Где Phase 7 даёт «exit 0», тут просто «всё зелёное», но цифры (N тестов, K файлов) — оставлять.
- При статусе **INCOMPLETE** / **NEEDS_INPUT** раздел «Что сделано» честно описывает, докуда дошли; раздел «Что нужно проверить тебе» превращается в «Что решить / разблокировать» с конкретными вопросами.
- Не извиняться, не благодарить, не использовать эмодзи (если только сам владелец явно не попросил).
